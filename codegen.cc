...
void CodeGen::visitProg(Prog *prog)
{
    ...
    //make sure enough space is allocated for globals
...
void CodeGen::visitGlobal(Global *global)
{
    ...
    //when symbols.insert(Symbol make sure that the arguments : name, type and the offset (displacement) are correct
    if(symbols.exists(currid)) {
        throw Redeclared(currid);
    }

    symbols.insert(Symbol(currid, currtype, symbols.numvars() + 3));
}
...
{
  //DTJ
    int looploc = code.pos(); // Get start of loop 
